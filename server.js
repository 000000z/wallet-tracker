...
server.on("upgrade", (request, socket, head) => {
  ...
  if (pathname === "/ws") {
    ...
    wssPumpfun.handleUpgrade(request, socket, head, (ws) => {
      wssPumpfun.emit("connection", ws, request);
    });
  } else if (pathname === "/ws-afk") {
    wssAfk.handleUpgrade(request, socket, head, (ws) => {
      wssAfk.emit("connection", ws, request);
    });
  } else if (pathname === "/ws-fourmeme") {
    wssFourmeme.handleUpgrade(request, socket, head, (ws) => {
      wssFourmeme.emit("connection", ws, request);
    });
  } else {
    socket.destroy();
  }
...
// CORS for GitHub Pages → Railway API calls
...
// ─── Pump.fun GitHub Token Tracker ────────────────────────────────────────────
let pumpfunTracker = null;
try {
  pumpfunTracker = require("./site/pumpfun-tracker");
...
// Pump.fun WebSocket (UI clients)
wssPumpfun.on("connection", (ws) => {
  console.log("Pump.fun WebSocket client connected");
  ws.isAlive = true;
  ...
  const status = pumpfunTracker ? pumpfunTracker.getStatus() : { running: false };
  ws.send(JSON.stringify({
    type: "info",
    msg: `Connected to pump.fun tracker. Status: ${status.running ? "Running" : "Stopped"}`,
    ts: new Date().toISOString(),
  }));
});

// Heartbeat for pump.fun WS clients
const pumpfunHeartbeat = setInterval(() => {
  wssPumpfun.clients.forEach((ws) => {
    if (!ws.isAlive) return ws.terminate();
    ws.isAlive = false;
    ws.ping();
  });
}, 25000);
wssPumpfun.on("close", () => clearInterval(pumpfunHeartbeat));

// Pump.fun API routes
app.post("/api/pumpfun/start", (req, res) => {
  if (!pumpfunTracker) return res.status(500).json({ ok: false, error: "Tracker module not loaded" });
  try {
    pumpfunTracker.startTracker();
    res.json({ ok: true, status: "running" });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
  }
...
// ─── AFK Creator Sniper (Pump.fun launch sniper) ─────────────────────────────
let afkSniper = null;
try {
  afkSniper = require("./site/afk-sniper");
...
// AFK WebSocket (UI clients)
wssAfk.on("connection", (ws) => {
  console.log("AFK Sniper WebSocket client connected");
  ws.isAlive = true;
  ws.on("pong", () => { ws.isAlive = true; });
  ...
  const status = afkSniper ? afkSniper.getStatus() : { running: false };
  ws.send(JSON.stringify({
    type: "info",
    msg: `Connected to AFK Sniper. Status: ${status.running ? "Running" : "Stopped"}`,
    ts: new Date().toISOString(),
  }));
});

// Heartbeat for AFK WS clients
const afkHeartbeat = setInterval(() => {
  wssAfk.clients.forEach((ws) => {
    if (!ws.isAlive) return ws.terminate();
    ws.isAlive = false;
    ws.ping();
  });
}, 25000);
wssAfk.on("close", () => clearInterval(afkHeartbeat));

// AFK API routes
app.post("/api/afk/start", async (req, res) => {
  if (!afkSniper) return res.status(500).json({ ok: false, error: "AFK Sniper module not loaded" });
  try {
    await afkSniper.startSniper();
    res.json({ ok: true, status: "running" });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
  }
...
app.get("/api/afk/status", async (req, res) => {
  ...
  const status = afkSniper.getStatus();
  // Refresh balance live
  status.balance = await afkSniper.refreshBalance();
  res.json(status);
});
...
app.post("/api/afk/config", (req, res) => {
  if (!afkSniper) return res.status(500).json({ ok: false, error: "AFK Sniper module not loaded" });
  afkSniper.updateConfig(req.body);
  res.json({ ok: true });
});
...
// ─── Four.Meme Agent Sniper (BSC) ─────────────────────────────────────────────
let fourmemeSniper = null;
try {
  fourmemeSniper = require("./site/fourmeme-sniper");
...
// Four.Meme WebSocket (UI clients)
wssFourmeme.on("connection", (ws) => {
  console.log("Four.Meme Sniper WebSocket client connected");
  ws.isAlive = true;
  ws.on("pong", () => { ws.isAlive = true; });
  ...
  const status = fourmemeSniper ? fourmemeSniper.getStatus() : { running: false };
  ws.send(JSON.stringify({
    type: "info",
    msg: `Connected to Four.Meme Sniper. Status: ${status.running ? "Running" : "Stopped"}`,
    ts: new Date().toISOString(),
  }));
});

// Heartbeat for Four.Meme WS clients
const fourmemeHeartbeat = setInterval(() => {
  wssFourmeme.clients.forEach((ws) => {
    if (!ws.isAlive) return ws.terminate();
    ws.isAlive = false;
    ws.ping();
  });
}, 25000);
wssFourmeme.on("close", () => clearInterval(fourmemeHeartbeat));

// Four.Meme API routes
app.post("/api/fourmeme/start", async (req, res) => {
  ...
  try {
    await fourmemeSniper.startSniper();
    res.json({ ok: true, status: "running" });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
  }
...
app.get("/api/fourmeme/status", async (req, res) => {
  ...
  const status = fourmemeSniper.getStatus();
  status.balance = await fourmemeSniper.refreshBalance();
  res.json(status);
});
...
app.post("/api/fourmeme/config", (req, res) => {
  if (!fourmemeSniper) return res.status(500).json({ ok: false, error: "Four.Meme Sniper module not loaded" });
  fourmemeSniper.updateConfig(req.body);
  res.json({ ok: true });
});
...
function shutdown() {
  ...
  stopSniper();
  if (pumpfunTracker) {
    try { pumpfunTracker.stopTracker(); } catch {}
  }
  if (afkSniper) {
    try { afkSniper.stopSniper(); } catch {}
  }
  if (fourmemeSniper) {
...
server.listen(PORT, () => {
  ...
  console.log(`Pump.fun: http://localhost:${PORT}/pumpfun-tracker.html`);
  console.log(`AFK:      http://localhost:${PORT}/afk-sniper.html`);
  console.log(`4Meme:    http://localhost:${PORT}/fourmeme-sniper.html`);
  console.log(`WS:       ws://localhost:${PORT}/ws`);
  console.log(`WS PF:    ws://localhost:${PORT}/ws-pumpfun`);
  console.log(`WS AFK:   ws://localhost:${PORT}/ws-afk`);
  console.log(`WS 4Meme: ws://localhost:${PORT}/ws-fourmeme`);
});
